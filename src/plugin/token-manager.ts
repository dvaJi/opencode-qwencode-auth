...
class TokenManager {
    ...
    try {
      ...
      if (
        ...
        debugLogger.info("Returning from memory cache", {
          ...
          validUntil: new Date(
            this.memoryCache.credentials.expiryDate!,
          ).toISOString(),
        });
      ...
      if (this.refreshPromise) {
        ...
        debugLogger.info("Wait completed", {
          success: !!result,
          age: Date.now() - startTime,
        });
        return result;
      ...
      this.refreshPromise = (async () => {
        ...
        const shouldCheckFile =
          forceRefresh || now - this.lastFileCheck >= CACHE_CHECK_INTERVAL_MS;
   ...
   */
  private async performTokenRefreshWithLock(
    current: QwenCredentials | null,
  ): Promise<QwenCredentials | null> {
    const credPath = getCredentialsPath();
    ...
    if (!lockAcquired) {
      ...
      if (reloaded && this.isTokenValid(reloaded)) {
        ...
        debugLogger.info(
          "Loaded refreshed credentials from file (multi-process)",
        );
        return reloaded;
      }
  ...
  getState(): {
    ...
    return {
      ...
      memoryCacheValid: this.memoryCache.credentials
        ? this.isTokenValid(this.memoryCache.credentials)
        : false,
